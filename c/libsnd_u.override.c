                   ...
                   int nsamples, SOUNDIN *p)
{
  ...
  int   n, ntot = 0;
  do {
    printf("infd: %d nsamples %d totalSamples %d minus %d \n", infd, nsamples, ntot, nsamples - ntot);
    n = sf_read_double(infd, inbuf + ntot, nsamples - ntot);
    printf("SF STRERR: %s\n", sf_strerror(infd));
    /* int n2 = read(infd, inbuf, 1024); */
    /* printf("N2 read %d errno %d \n", n2, errno); */
    printf("POST infd: %d nsamples %d totalSamples %d \n", infd, nsamples, ntot);
    printf("N RETURN FROM sf_read %d casted %d \n", n, (int) n);
    if (UNLIKELY(n < 0))
...
printf("skipframes %d \n", skipframes);
  if (skipframes < 0) {
    ...
    printf("sreadin 1: p->fd %d p->sinfd %d \n", p->fd, p->sinfd);
