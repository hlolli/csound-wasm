...
struct wasiFSFile wasiFSFiles[256];

void setupWasmBrowserFS(void) {
  unsigned short int i;
  for (i = 1; i <= 256; i++) {
    wasiFSFiles[i].isEmpty = true;
    wasiFSFiles[i].isBroken = false;
    wasiFSFiles[i].index = i;
    wasiFSFiles[i].fd = -1;
  }
}

/* CSOUND-WASM MODIFIED FIND_FD */
int findAvailableEmptyFileIndex(void) {
  int i;
  for (i = 1; i <= 256; i++) {
    if (wasiFSFiles[i].isEmpty == true &&
        wasiFSFiles[i].isBroken == false) {
      return i;
    }
  }
...
struct wasiFSFile getWasiFileFromFd(int fd) {
  int i;
  for (i = 1; i <= 256; i++) {
    if (wasiFSFiles[i].fd == fd) {
      return wasiFSFiles[i];
    }
  }
}
...
void *maybeAppendSandboxPrefix(const char *filename, char *fullPath) {
  if (startsWith("/sandbox/", filename)) {
    strcpy(fullPath, filename);
  } else if (startsWith("/", filename)) {
    strcpy(fullPath, "/sandbox");
...
{                                               /* (not called if nosound)  */
    ...
    if (STA(pipdevout)) {
      ...
      int fileIndex = findAvailableEmptyFileIndex();
      sprintf(fullName, "/sandbox/%d", fileIndex);
      ...
      csoundNotifyFileOpened(csound, fName, type2csfiletype(O->filetyp, O->outformat), 1, 0);
    }
